...
define_multipurpose_modmap(
    ...
    # {Key.LEFT_: [Key.ESC, Key.RIGHT_CTRL] # Caps2Esc - Chromebook
    {                                         # Placeholder
})
...
define_keymap(re.compile("^jetbrains-(?!.*toolbox).*$", re.IGNORECASE),{
    ...
    # Navigation
    K("C-o"): K("C-n"),                         # Go to classs
    K("C-Shift-o"): K("C-Shift-n"),             # Go to file
    K("C-M-o"): K("C-M-Shift-n"),               # Go to symbol
    K("RC-Shift-Right_Brace"): K("Alt-Right"),             # Go to next editor tab
    K("RC-Shift-Left_Brace"): K("Alt-Left"),               # Go to previous editor tab
